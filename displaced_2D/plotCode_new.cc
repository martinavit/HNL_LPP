...
void plotDataVSMC(int categoria, TH1D* data, TH1D** bkg, const TString* names, const unsigned nHist, const TString& file,const TString& file2, const bool ylog, const unsigned widthopt, const bool plotsig, TH1D** signal , const TString* signames, const unsigned nSig, const bool signorm){
	...
	if(plotsig){
		...
		for(unsigned sig = 0; sig < nSig; ++sig){
		  	if (sig == 2 || sig == 6 || sig == 7 || sig == 8 || sig == 5 || sig == 4  || sig == 10) continue;
    ...
    if(plotsig){
      for(unsigned sig = 0; sig < nSig; ++sig){
	if ( sig != 2 &&  sig != 5 && sig != 7 && sig != 8 &&  sig != 6 && sig != 4 && sig != 10) {
	  if(signorm && signal[sig]->GetSumOfWeights() != 0) signal[sig]->Scale(bkgTot->GetSumOfWeights()/ signal[sig]->GetSumOfWeights());
	  signal[sig]->SetMinimum(0.1);
