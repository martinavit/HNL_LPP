...
void Analysis_mc::analisi(int selezione, int num_histo_kin
  ...
  //******************* HISTO **********************
  const int nCat=24;
  const int nDist = 9;  //Number of distributions to plo
  const int nHisto_per_plot = 5;

  TH2D* Histos[nDist][nCat][4];

  const TString nameplots[nHisto_per_plot] = {"bgk","x_gamma", "1gev","4gev","9gev"};

  const TString catNames[nCat]= {"_0", "_1", "_2", "_final", "_0_ossf", "_1_ossf", "_2_ossf","_final_ossf",  "_0_no_ossf", "_1_no_ossf", "_2_no_ossf","_final_no_ossf", "__prompt__0", "__prompt__1", "__prompt__2","__prompt_final",  "__prompt__0_ossf", "__prompt__1_ossf", "__prompt__2_ossf","__prompt_final_ossf", "__prompt__0_no_ossf", "__prompt__1_no_ossf", "__prompt__2_no_ossf", "__prompt_final_no_ossf"};
  const TString Histnames_ossf_2D[nDist] = {"DeltaR_st_vs_minDeltaPhi","DeltaR_st_vs_DeltaR_lt","DeltaR_st_vs_pt_s","DeltaR_st_vs_pt_t",
					    "DeltaPhi_st_vs_minDeltaPhi","DeltaPhi_st_vs_DeltaR_st", "DeltaPhi_st_vs_DeltaR_lt",
					    "X_Y_vertex", "X_Y_vertex23"};

  const TString Xaxes[nDist] = {"#Delta R (l_{2}-l_{3})","#Delta R (l_{2}-l_{3})","#Delta R (l_{2}-l_{3})","#Delta R (l_{2}-l_{3})",
				"#Delta #phi (l_{2}-l_{3})","#Delta #phi (l_{2}-l_{3})","#Delta #phi (l_{2}-l_{3})", 
				"vtx_X (cm)", "vtx_X (cm)"};
  const TString Yaxes[nDist] = {"Min #Delta #phi (l_{1}-other)","#Delta R (l_{1}-l_{3})","p_{T} (l_{2})","p_{T} (l_{3})",
				"Min #Delta #phi (l_{1}-other)","#Delta R (l_{2}-l_{3})","#Delta R (l_{1}-l_{3})",
				"vtx_Y (cm)","vtx_Y (cm)" };

  const TString XUnits[nDist] = {"","","","","","","", "", ""};
  const TString YUnits[nDist] = {"","","GeV","GeV","","","", "", ""};

				
  const double XHistMin[nDist] = {0,0,0,0,0,0,0, -200, -200};
  const double XHistMax[nDist] = {4.5,4.5,4.5,4.5,3.3,3.3,3.3, 200,200};
  const double YHistMin[nDist] = {0,0,0,0,0,0,0, -200, -200};
  const double YHistMax[nDist] = {3.3,4.5,100,100,3.3,4.5,4.5, 200,200};
		
  const int XnBins[nDist] =      {20,20,20,20,30,30,30, 200, 200};    
  const int YnBins[nDist] =      {30,20,25,25,30,20,20, 200, 200};     
 
  cout<<"------ 1"<<endl;
  for(int i = 0; i < nDist; ++i){
    for(int effsam = 0; effsam < nHisto_per_plot; ++effsam){
      for(int cat = 0; cat < nCat; ++cat){               
	Histos[i][cat][effsam] = new TH2D(catNames[cat] +"_"+nameplots[effsam] +"_" + Histnames_ossf_2D[i] , catNames[cat] +nameplots[effsam] + Histnames_ossf_2D[i], XnBins[i], XHistMin[i], XHistMax[i], YnBins[i], YHistMin[i], YHistMax[i]);
  ...
  for(int sam = 0, effsam = 0; sam < nSamples; ++sam, ++effsam){
	...
	_trackIso[index_l[0]] =  _trackIso[index_l[0]] - pt_cone_trailing;
      ...
      double Xvalues[nDist] ={lepton_reco[1].DeltaR(lepton_reco[2]), lepton_reco[1].DeltaR(lepton_reco[2]),lepton_reco[1].DeltaR(lepton_reco[2]),lepton_reco[1].DeltaR(lepton_reco[2]),
			      lepton_reco[1].DeltaPhi(lepton_reco[2]),lepton_reco[1].DeltaPhi(lepton_reco[2]),lepton_reco[1].DeltaPhi(lepton_reco[2]),
			      _vertex_X[2],_vertex_X[2] };



      double Yvalues[nDist] ={min_delta_phi, lepton_reco[0].DeltaR(lepton_reco[2]),  pt_cone_sub_leading, pt_cone_trailing,
			      min_delta_phi, lepton_reco[1].DeltaR(lepton_reco[2]),  lepton_reco[0].DeltaR(lepton_reco[2]),
			      _vertex_Y[2], _vertex_Y[2]};
	...
	if (selection_0)      Histos[numero_histo][0][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo], scal);
	if (selection_1)      Histos[numero_histo][1][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo],  scal);
	if (selection_2)      Histos[numero_histo][2][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo],  scal);
	if (ossf_event ){
	  if (selection_0)      Histos[numero_histo][3][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo], scal);
	  if (selection_1)      Histos[numero_histo][4][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo],  scal);
	  if (selection_2)      Histos[numero_histo][5][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo],  scal);
	}//end ossf
	if (!ossf_event ){
	  if (selection_0)      Histos[numero_histo][6][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo], scal);
	  if (selection_1)      Histos[numero_histo][7][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo],  scal);
	  if (selection_2)      Histos[numero_histo][8][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo],  scal);
	}//end ossf

	if (_prompt){
	  if (selection_0)      Histos[numero_histo][9][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo], scal);
	  if (selection_1)      Histos[numero_histo][10][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo],  scal);
	  if (selection_2)      Histos[numero_histo][11][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo],  scal);
	  if (ossf_event ){
	    if (selection_0)      Histos[numero_histo][12][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo], scal);
	    if (selection_1)      Histos[numero_histo][13][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo],  scal);
	    if (selection_2)      Histos[numero_histo][14][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo],  scal);
	  }//end ossf
	  if (!ossf_event ){
	    if (selection_0)      Histos[numero_histo][15][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo], scal);
	    if (selection_1)      Histos[numero_histo][16][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo],  scal);
	    if (selection_2)      Histos[numero_histo][17][fill]->Fill(Xvalues[numero_histo],Yvalues[numero_histo],  scal);
	  }//end ossf
	}//end prompt
      }//end histo
  ...
  for (int i =0; i < 4; i++){
    for (int cat = 0; cat < nCat; cat++){
      for(int numero_histo = 0; numero_histo < nDist; ++numero_histo){
	Histos[numero_histo][cat][i] -> SetAxisRange(XHistMin[numero_histo],XHistMax[numero_histo],"X");
	Histos[numero_histo][cat][i] -> SetAxisRange(YHistMin[numero_histo],YHistMax[numero_histo],"Y");
	Histos[numero_histo][cat][i] -> SetXTitle(Xaxes[numero_histo]);
	Histos[numero_histo][cat][i] -> SetYTitle(Yaxes[numero_histo]);
      }
    }
  }

  TCanvas *c[nDist][nCat];
  for (int cat = 0; cat < nCat; cat++){
    for(int numero_histo = 0; numero_histo < nDist; ++numero_histo){
      c[numero_histo][cat] = new TCanvas("c_"+catNames[cat]+"__"+Histnames_ossf_2D[numero_histo],"c_"+catNames[cat]+"__"+Histnames_ossf_2D[numero_histo],10,10,700,700);
      drawLumi(c[numero_histo][cat]);
      c[numero_histo][cat]->Divide(2,2);
      for (int i =0; i < 4; i++){
	c[numero_histo][cat]-> cd (i+1);
	Histos[numero_histo][cat][i] -> Draw("COLZ");
      }
      c[numero_histo][cat]->Print("plots_pdf/"+catNames[cat]+"/"+catNames[0]+"_"+ Histnames_ossf_2D[numero_histo]+".pdf");
      c[numero_histo][cat]->Print("plots_root/"+catNames[cat]+"_"+ Histnames_ossf_2D[numero_histo]+".root");
      delete c[numero_histo][cat];
    }
  }
